...
export default function SignupPage() {
  ...
  const handleSignup = async (event: React.FormEvent) => {
    ...
    let signUpResp: SignUpResponse | null
    try {
      // Proxy signups through our server endpoint to enforce rate limits and auditing
      const resp = await fetch('/api/auth/signup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: trimmedEmail, password, full_name: trimmedName }),
      })
      signUpResp = await resp.json().catch(() => null)
  ...
  return (
    <>
      ...
      <AuthCard title="Create your Plugcraft account" subtitle="Verify once, download forever. Set your credentials and unlock Ghosted." leftGraphic={<Image src="/assets/software_logo.png" alt="Illustration" width={240} height={240} className="rounded-md"/>}>
        <form onSubmit={handleSignup} className="mt-2 space-y-5">
            ...
            {feedback && (
              <p role="status" aria-live="polite" className={`text-sm ${feedback.type === 'error' ? 'text-rose-200' : 'text-emerald-200'}`}>
                {feedback.message}
              </p>
            )}
            <button
